...
def get_data(category=None,user=None,page_no=0,limit=3):
	...
	query = """\
		...
		from `tabDiscussion Topic` t1, `tabEmployee` t2, `tabToDo` t3
	...
	posts = frappe.db.sql(query+ limit_query, as_dict=1,debug=1)
...
def list_of_assigned_topics():
	return frappe.db.sql("""select distinct t1.name, t1.title ,t1.published_on from `tabDiscussion Topic`t1,`tabToDo`t2 
				where t2.reference_name = t1.name and t2.owner = '{0}'""".format(frappe.session.user),as_list=1,debug=1)	
